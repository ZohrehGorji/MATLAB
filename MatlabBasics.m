...
C=[zeros(3,3), M;M,zeros(3,3);zeros(3,3),M];
M_9x15=[repmat(C,1,2), [zeros(3,3); M;zeros(3,3)]];
%M_9x15 = zeros(9,15);
%M_9x15(1:3,1:3) = M;
%M_9x15(1:3,7:9) = M;
%M_9x15(4:6,4:6) = M;
...
%v2=3x1
%m=3x3
function[result] = vector_X_Matrix(v1, M) %1x3 und 3x3 = 1x3

% return a row vector
     
result = zeros(1, size(M, 2)); %zuerst mit null fill. 1x3 weil msize1=3


    for i = 1:size(M, 2) %von spalte1 bis spalte 3 (size(M,2)=anzahl der spalten ) 
        for j = 1:size(M, 1) %von zeile 1 bis zeile 3 (size(M,1)=anzahl der spalten ) 
            result(i) = result(i) + v1(j)* M(i, j);  % result(spalte) = result(spalte) + (v1(zeile)*M(zeile,spalte));
...
%return a column vector
 ...
 result = zeros(size(M, 1), 1);
    ...
    for i = 1:size(M, 1)% 1 bis 3%zeilen
        for j = 1:size(M, 2) % 1 bis 3%spalten
            result(i) = result(i) + M(i, j) * v2(j);    % result(row) = result(row) + (M(row,column)*v2(column));
